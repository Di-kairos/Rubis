...
- Последняя сессия: **12** (2026-08-12: CUE влит в `main`, D-013, audio-verify
  24/24) — отчёт `docs/sessions/progress-report-session11.md` за прошлую
- Ветка `main`, head — см. `PROGRESS.md`. Тесты **193/193**, Debug без warnings
- **CUE закрыт по коду**: `phase/09-cue` слита (`33ab227`), `audio-verify`
  24/24 на обеих ветках. Прошлое «0/24» было отсутствием TCC на захват, не
  регрессом — перед прогоном смотреть строку `mic permission status` (нужно 3).
  Осталось живое: рип «диск одним FLAC + .cue», границы на слух
- **Опубликована Rubis Music 0.9.0** (build 27); в `main` с тех пор накопилось
  невыпущенное — вёрстка, D-010 (MIT), D-011, D-012, лого, CUE (D-013)
