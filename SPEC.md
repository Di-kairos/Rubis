...
6. **DSD.** DSF и DSDIFF. DoP-пакеты как есть — только если владелец подтвердил DoP для
   этого устройства по UID и один физический формат выхода даёт частоту, ≥24 бит и стерео
   разом (D-014: HAL не умеет спросить ЦАП, разбирает ли он DoP-маркеры).
