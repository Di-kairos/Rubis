...
- Стриминговые сервисы (Spotify, Tidal, Qobuz, Apple Music).
- Редактирование тегов (чтение — да, запись — нет).
- Эквалайзер, DSP, room correction, upsampling.
...
- Last.fm скробблинг (кандидат на v2, но структура БД `play_history` должна его допускать).
...
- Никакой автообновляемости, никаких проверок версий.
