...
struct AuditRegressionTests {
    @Test func compensatedCueTimeMustStartAtZero() throws {
        ...
        let url = try #require(
            ...
            ).filter { $0.pathExtension == "flac" }.sorted { $0.path < $1.path }.first)
        ...
        let region = try #require(
            CueRegion(
                track: Track(
                    sourceId: "audit", title: "Audit", duration: 1, codec: "flac",
                    sampleRate: Int(rate), cueStart: 2, cueEnd: 3)))
        let decoder = try region.decoder(url: url)
        if !decoder.isOpen { try decoder.open() }
        print(
            "AUDIT CUE time: framePosition=\(decoder.position), frameLength=\(decoder.length), expected=\(Int(rate))"
        )
    ...
    @Test func shuffleMustKeepDuplicateQueueEntries() {
        let a = PlaybackItem(
            track: Track(
                id: 1, sourceId: "audit", title: "A", duration: 5, codec: "flac", sampleRate: 44100),
            url: URL(fileURLWithPath: "/audit-a.flac"))
        let b = PlaybackItem(
            track: Track(
                id: 2, sourceId: "audit", title: "B", duration: 5, codec: "flac", sampleRate: 44100),
            url: URL(fileURLWithPath: "/audit-b.flac"))
        var random = SystemRandomNumberGenerator()
        let result = PlaybackOrder.shuffled(
            items: [a, b, a], current: a, mode: .tracks, using: &random)
